...
function createTopic(topicName: string, creator: string, voters: string[], votes: number = -1): Topic {
  let topic = {
    topicName: topicName,
    creator: creator,
    voters: voters,
...
async function _storeTopics(storageKey: string, updateFun) {
      ...
      let outTopics = updateFun(previousTopics);
...
export function useStorage(spaceKey: string, contentId: string, localId: string) {
  ...
  let [topics, setTopics] = useState(() => fetchTopics(storageKey));
  return {
    topics,
